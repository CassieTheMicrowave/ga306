...
def create_body(length=2, width=1):
    ...
    body = cmds.polyPlane(w=length, h=width, name="body", subdivisionsX=200, subdivisionsY=200)
...
def create_tire(name, width, radius, tx, ty, tz):
    ...
    # Return the transform node name.
    timeStart = time.clock()
    tire = cmds.polyCylinder(h=width, r=radius, ax=(0,0,1), sc=True, name=name, subdivisionsX=50, subdivisionsY=50)
